...
<div class="container">
    ...
    <form method="POST">
        <!-- Input for filtering books -->
        <label for="filter">Filter Books by Name:</label>
        <input type="text" id="filter" onkeyup="filterBooks()" placeholder="Type a letter or part of the title..." />

        <label for="book_id">Select Book:</label>
        <select name="book_id" id="book_id" required>
            <option value="">-- Select a Book --</option>
            <?php while ($book = $books->fetch_assoc()) : ?>
                <option value="<?= $book['book_id'] ?>" class="book-option">
...
<script>
    function filterBooks() {
        ...
        options.forEach(option => {
            const bookTitle = option.textContent.toLowerCase();
            if (bookTitle.indexOf(filter) > -1) {
                option.style.display = ''; // Show matching books
            } else {
                option.style.display = 'none'; // Hide non-matching books
            }
        });
    }
</script>
